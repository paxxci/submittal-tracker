...
export default function SubmittalDetailPanel({ submittal, projectId, activeUser, activeUserRole, onClose, onUpdated }) {
  ...
  const renderLogEntry = (entry, isPinned = false) => {
    ...
    displayMsg = displayMsg.replace(/O&M Document uploaded: "(.+?)"/, '📕 O&M Document: Uploaded - "$1"')
    displayMsg = displayMsg.replace(/Reference File uploaded: "(.+?)"/, '🔗 Reference File: Uploaded - "$1"')
    displayMsg = displayMsg.replace(/🔄 Re-classified "(.+?)" to Revision (\d+)/, '🔄 Submittal: Changed to Rev $2 "$1"')
    displayMsg = displayMsg.replace(/✅ Stamped \[R(\d+)\] "(.+?)" as Officially Approved Version/, '✅ Submittal: Approved - Rev $1 "$2"')
    displayMsg = displayMsg.replace(/⏪ Revoked Approval Stamp from \[R(\d+)\] "(.+?)"/, '⏪ Submittal: Revoked Approval - Rev $1 "$2"')
    displayMsg = displayMsg.replace(/🗑️ Deleted Document: "(.+?)"/, '🗑️ Document: Deleted - "$1"')
    displayMsg = displayMsg.replace(/🚀 Submittal Bumped to Revision (\d+)/, '🚀 Submittal: Bumped to Rev $1')
    displayMsg = displayMsg.replace(/🚀 Submittal Revision manually set to (\d+)/, '🚀 Submittal: Revision Override to Rev $1')
    ...
    <div className={`detail-panel open`} id="detail-panel">
      ...
      <div className="detail-panel-body" style={{ display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', width: '100%', flexShrink: 0, background: 'var(--bg-base)', borderBottom: '1px solid var(--border)' }}>
          <Tab id="tab-details" active={activeTab === 'details'} onClick={() => setActiveTab('details')}>Details</Tab>
          <Tab id="tab-activity" active={activeTab === 'activity'} onClick={() => setActiveTab('activity')}>Activity</Tab>
          <Tab id="tab-documents" active={activeTab === 'documents'} onClick={() => setActiveTab('documents')}>Documents</Tab>
        </div>
